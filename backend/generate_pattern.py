...
def remove_background(img, bg_color, threshold=30):
    ...
    Only removes background pixels connected to the image edges.
    Internal white areas (e.g., eye whites, highlights) are preserved.
    
    Returns (rgba_image, bg_mask) where bg_mask[y][x] = True for background cells.
    """
    img_rgb = img.convert('RGB')
    w, h = img_rgb.size
    
    # Build a visited set for flood fill
    from collections import deque
    
    br, bg_ref, bb = bg_color
    
    def is_bg(px):
        r, g, b = px
        # Tighter threshold: each channel within 25 of bg, total diff < 50
        return (abs(r - br) < 25 and abs(g - bg_ref) < 25 and abs(b - bb) < 25
                and abs(r - br) + abs(g - bg_ref) + abs(b - bb) < 50)
    ...
    for x in range(w):
        px = img_rgb.getpixel((x, 0))[:3]
        if is_bg(px):
            queue.append((x, 0))
            visited.add((x, 0))
        px = img_rgb.getpixel((x, h - 1))[:3]
        if is_bg(px):
            queue.append((x, h - 1))
            visited.add((x, h - 1))
    
    for y in range(h):
        px = img_rgb.getpixel((0, y))[:3]
        if is_bg(px):
            queue.append((0, y))
            visited.add((0, y))
        px = img_rgb.getpixel((w - 1, y))[:3]
        if is_bg(px):
            queue.append((w - 1, y))
            visited.add((w - 1, y))
    ...
    bg_count = len(visited)
    print(f'🗑️  Flood-fill removed {bg_count} background pixels (connected to edges)')
...
def generate_pattern(image_path, output_path, grid_w=29, grid_h=29,
    ...
    if bg_remove and has_bg:
        if bg_color:
            img, bg_mask = remove_background(img, bg_color, bg_threshold)
        else:
            print('🗑️  Image already has transparent background')

    # ── Trim whitespace ──
    img, crop_bbox = trim_whitespace(img)
    
    # Adjust bg_mask after trim
    if bg_mask is not None and crop_bbox:
        x0, y0, x1, y1 = crop_bbox
        new_w, new_h = x1 - x0, y1 - y0
        new_mask = [[False] * new_w for _ in range(new_h)]
        for y in range(y0, y1):
            for x in range(x0, x1):
                if bg_mask[y][x]:
                    new_mask[y - y0][x - x0] = True
        bg_mask = new_mask
    ...
    grid_bg_mask = [[False] * grid_w for _ in range(grid_h)]  # Grid-level background mask
    ...
    for gy in range(grid_h):
        for gx in range(grid_w):
            ...
            if not pixels:
                ...
                grid_bg_mask[gy][gx] = True
                continue

            # Check if majority of this cell is background
            if bg_mask is not None:
                bg_px_count = 0
                for py in range(y0, y1):
                    for px in range(x0, x1):
                        if py < orig_h and px < orig_w and bg_mask[py][px]:
                            bg_px_count += 1
                if bg_px_count > len(pixels) * 0.5:
                    grid_bg_mask[gy][gx] = True
                    img_resized.putpixel((gx, gy), (255, 255, 255))
                    continue
    ...
    # ── Map colors and count usage (skip background cells) ──
    color_map = {}
    usage_counter = {}
    bg_bead_count = 0
    for y in range(grid_h):
        for x in range(grid_w):
            if grid_bg_mask[y][x]:
                bg_bead_count += 1
                color_map[(y, x)] = None  # Background cell
                continue
    ...
    subject_beads = total_beads - bg_bead_count
    ...
    stats_text = f"总豆数: {subject_beads}颗  |  网格: {grid_w}×{grid_h}  |  颜色: {n_colors}色"
    ...
    for y in range(grid_h):
        for x in range(grid_w):
            ...
            # Background cells: light gray fill, no symbol
            if entry is None:
                draw.rectangle(
    ...
    # Build color detail with RGB values
    colors_detail = []
    for cid, count in used_colors:
        entry = id_to_palette.get(cid, {})
        colors_detail.append({
            'id': cid,
            'count': count,
    ...
    return {
        ...
        'subject_beads': subject_beads,
        'bg_beads': bg_bead_count,
        'color_count': n_colors,
        'usage': used_colors,
        'colors_detail': colors_detail,
